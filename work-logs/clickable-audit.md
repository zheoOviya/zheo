...
Deliverable conventions: PASS = element works as expected; FAIL = real defect (dead href, no-op handler, crash, unknown endpoint); WARN = intentionally disabled initial state / degraded behavior that does not break the flow.

## Summary

| App | PASS | FAIL (fixed) | WARN | Total elements |
|---|---|---|---|---|
| Consumer | 28 | 4 (4 fixed) | 2 | 34 |
| Vendor | 14 | 0 | 3 | 17 |
| Admin | 16 | 2 (2 fixed) | 1 | 19 |
| API endpoints (frontend refs) | 26 | 1 (fixed) | 0 | 27 |
| **Total** | **84** | **7** | **6** | **97** |

Runtime route crawl: consumer 10/10, vendor 10/11 (`/login` 404, nothing links to it), admin 10/10. All auth-protected pages 307 → `/login` as designed.
...
| KDS Hand Over (pickup) | `page.tsx:297` → `:139-147` | POST `/api/v1/orders/:id/confirm-pickup` | route exists (fulfillmentRouter) | PASS |
| KDS OTP input | `page.tsx:281` | 4-digit pickup OTP | disabled until 4 digits | WARN |
| KDS Hand Over disabled state | `page.tsx:293` | disabled unless OTP complete | `disabled=…length !== 4` | WARN |
...
| Vendor `/login` | n/a | — | 404; nothing links to it (no vendor login page by design) | WARN/info |
...
| Sidebar Sign Out | `Sidebar.tsx:117` | clear local session + `/login` | `clearSession()` + redirect; no server logout call (admin token is localStorage-only, so functionally signed out) | WARN |
...
Vendor (3002): `/` 200, `/menu` 200, `/menu/bulk` 200, `/pos` 200, `/insights` 200, `/promotions` 200, `/settlements` 200, `/gst` 200, `/catering` 200, `/chain` 200, `/login` 404 (nothing links to it).
...
- Backend: 40 files / 379 tests pass (root `vitest.config.ts`).
