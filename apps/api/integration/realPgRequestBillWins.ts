// ============================================================
// I3.2 — Real-PG service-interleaving proof: requestBill-wins ordering.
//
...
//   pnpm exec tsx apps/api/integration/realPgRequestBillWins.ts
//
// PROVES (and only claims) the SINGLE requestBill-wins interleaving:
//   - DiningSessionService.requestBill and DineInOrderService.placeOrder run
...
//   - placeOrder, started after requestBill holds the lock, BLOCKS on the
...
//     BRING_BILL, the blocking placeOrder revalidates the now-frozen session
//     and is REJECTED with the accepted existing error contract
//     SESSION_CLOSED_FOR_ORDERING (409) — NO new order/item is persisted.
//   - the frozen bill artifacts are NOT altered by the failed placeOrder:
//     exactly 1 SessionBill, 1 BRING_BILL, session stays BILL_REQUESTED,
//     and the pre-existing order count N stays exactly N.
//
...
// real session lock is acquired (requestBill locks -> signal -> placeOrder
// starts and blocks -> requestBill freezes + commits -> placeOrder proceeds).
// NO production code is touched; the gate lives entirely inside this harness.
//
// DOES NOT claim (I4+ territory): cancel/advance interleavings, double
// requestBill concurrency, deadlock-freedom, rollback semantics, general
// MVCC correctness, durable event ordering / outbox / exactly-once.
// ============================================================
...
async function main(): Promise<void> {
  ...
  const poolA = new Pool({ connectionString: url, max: 1, application_name: "itrackQA" });
  const poolB = new Pool({ connectionString: url, max: 1, application_name: "itrackQB" });
  const poolC = new Pool({ connectionString: url, application_name: "itrackQC" });
  ...
  const tableToken = `itrack-rbw-${randomUUID().replace(/-/g, "")}`;
  const label = `I3.2-${randomUUID().slice(0, 8)}`;
  const gst = `GST${randomUUID().replace(/-/g, "").slice(0, 12)}`;
  const fssai = `FSSAI${randomUUID().replace(/-/g, "").slice(0, 12)}`;
  const correlationId = `i3.2-${randomUUID()}`;
  ...
  const itemName = `I3.2-Item-${randomUUID().slice(0, 8)}`;
  ...
  try {
    ...
    // one pre-existing non-CANCELLED billable order + item snapshot ----
    ...
    await setup.execute(sql`
      ...
      VALUES (${restaurantId}, ${ownerUserId}, ${`I3.2-Restaurant-${randomUUID().slice(0, 8)}`}, ${gst}, ${fssai}, true)
    `);
    await setup.execute(sql`
    ...
    await setup.execute(sql`
      ...
      VALUES (${preOrderId}, ${sessionId}, ${restaurantId}, ${ownerUserId}, 'PLACED', ${billTotal})
    `);
    await setup.execute(sql`
      INSERT INTO dine_in_order_items
        (id, dine_in_order_id, restaurant_id, menu_item_id, name, base_price, quantity, item_subtotal)
      VALUES (${randomUUID()}, ${preOrderId}, ${restaurantId}, ${menuItemId}, ${itemName}, ${menuPrice}, ${menuQty}, ${itemSubtotal})
    `);
    console.log(
      "FIXTURE OK: user + restaurant + table + menu_item + ACTIVE session + pre-existing order snapshot inserted",
    );
    console.log(
      `  EXPECTED FROZEN BILL: food_subtotal=${foodSubtotal} gst_food=${gstFood} total=${billTotal} (from pre-existing snapshot)`,
    );

    // ---- two genuinely independent service backends ----
    // portA (gated) -> DiningSessionService (requestBill).
    // portB -> DineInOrderService (placeOrder).
    ...
    const aPid = await backendPid(poolC, "itrackQA");
    const aAct = await backendActivity(poolC, "itrackQA");
    if (!aPid) throw new Error("INDEPENDENCE FAILED: could not identify requestBill backend (itrackQA)");
    console.log(
      `[C] requestBill holds session lock; backend pid=${aPid} state=${aAct.state} (paused inside real tx)`,
    );

    // ============================================================
    // STEP 2 — placeOrder starts and BLOCKS on the same row.
    // ============================================================
    const placeOrderPromise = serviceB.placeOrder({
      session_id: sessionId,
      caller_user_id: ownerUserId,
      correlation_id: correlationId,
    ...
    const bAct = await waitUntilBlocked(poolC, "itrackQB", 8000);
    const bPid = await backendPid(poolC, "itrackQB");
    if (!bPid) throw new Error("INDEPENDENCE FAILED: could not identify placeOrder backend (itrackQB)");
    if (aPid === bPid) throw new Error(`INDEPENDENCE FAILED: same backend pid ${aPid}`);
    console.log(
      `[C] placeOrder backend blocked: wait_event_type=${bAct.wait_event_type} wait_event=${bAct.wait_event} state=${bAct.state} pid=${bPid}`,
    );

    const bSettled = await isSettled(placeOrderPromise);
    if (bSettled) {
      throw new Error("PLACEORDER BLOCKING FAILED: placeOrder settled while requestBill held the session lock");
    }
    console.log("PLACEORDER BLOCKING: placeOrder pending (not resolved, no error) while requestBill holds");

    const locks = await observeLocks(poolC, aPid, bPid);
    console.log(`pg_locks while placeOrder blocked: ${JSON.stringify(locks)}`);
    ...
    if (!bXid || bXid.granted) throw new Error("LOCK ASSERT FAIL: placeOrder must show NOT-granted transactionid ShareLock");
    if (!bTuple || !bTuple.granted) throw new Error("LOCK ASSERT FAIL: placeOrder must hold granted tuple AccessExclusiveLock");
    console.log(
      "LOCK ASSERT: requestBill transactionid ExclusiveLock granted=true; placeOrder transactionid ShareLock granted=false + tuple AccessExclusiveLock granted=true",
    );
    ...
    console.log(
      `[A] requestBill committed: session=${session.status} bill_total=${bill.total_amount} bringBill=${bringBillRequest?.request_type}`,
    );
    ...
    // ============================================================
    // STEP 4 — placeOrder proceeds after requestBill commit and is REJECTED.
    // ============================================================
    let placeOrderError: AppError | null = null;
    let placeOrderResult: unknown = null;
    try {
      placeOrderResult = await withTimeout(
        placeOrderPromise,
        10000,
        "placeOrder did not complete after requestBill commit",
      );
    } catch (err) {
      placeOrderError = err instanceof AppError ? err : null;
      if (!placeOrderError) {
        throw new Error(`placeOrder rejected with unexpected error type: ${String(err)}`);
      }
    }
    if (placeOrderError === null) {
      throw new Error(`placeOrder unexpectedly SUCCEEDED after bill freeze: ${JSON.stringify(placeOrderResult)}`);
    }
    console.log(
      `[B] placeOrder rejected post-freeze: code=${placeOrderError.code} status=${placeOrderError.status}`,
    );
    assertEqual(placeOrderError.code, "SESSION_CLOSED_FOR_ORDERING", "placeOrder rejection code (accepted existing contract)");
    assertEqual(placeOrderError.status, 409, "placeOrder rejection status");

    // ============================================================
    // STEP 5 — fresh committed reads: no new order/item, frozen artifacts intact.
    // ============================================================
    ...
    const orderRows = (await poolC.query(
      `SELECT id, status, total_amount FROM dine_in_orders WHERE session_id = $1 ORDER BY created_at, id`,
      [sessionId],
    )).rows as { id: string; status: string; total_amount: string }[];
    // N existing orders before the race == N after (no new order persisted).
    assertEqual(orderRows.length, 1, "dine_in_orders cardinality (N before == N after, no new order)");
    assertEqual(orderRows[0]!.id, preOrderId, "only the pre-existing order remains");
    assertEqual(orderRows[0]!.status, "PLACED", "pre-existing order status unchanged");
    assertNum(Number(orderRows[0]!.total_amount), billTotal, "pre-existing order total unchanged");
    ...
    assertEqual(itemRows.length, 1, "dine_in_order_items cardinality (no new item persisted)");
    assertEqual(itemRows[0]!.quantity, menuQty, "pre-existing order item quantity unchanged");
    assertNum(Number(itemRows[0]!.item_subtotal), itemSubtotal, "pre-existing order item_subtotal unchanged");
    ...
    assertNum(Number(billRow.food_subtotal), foodSubtotal, "frozen bill food_subtotal unchanged");
    assertNum(Number(billRow.gst_food), gstFood, "frozen bill gst_food unchanged");
    assertNum(Number(billRow.packaging_fee), 0, "frozen bill packaging_fee unchanged");
    assertNum(Number(billRow.gst_packaging), 0, "frozen bill gst_packaging unchanged");
    assertNum(Number(billRow.total_amount), billTotal, "frozen bill total_amount unchanged");
    ...
    console.log("I3.2 REQUESTBILL-WINS PROOF OK");
  } catch (err) {
    console.error("I3.2 PROOF FAILED:", err instanceof Error ? err.message : err);
    process.exitCode = 1;
  } finally {
