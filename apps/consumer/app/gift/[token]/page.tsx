...
export default function GiftClaimPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // unlike AuthGate). The claim gate keys on accessToken, not isAuthenticated,
  // so it works whether or not the refresh set the flag.
  useEffect(() => {
    if (isAuthenticated) {
  ...
  async function handleClaim() {
    if (!token) return;
    // Key on accessToken (not isAuthenticated): an auth check against
    // isAuthenticated could bounce a valid session back to /login if the flag
    // lags the token (e.g. hydration races), so the token is the source of truth.
    if (!accessToken) {
